...
impl SendAdditionalConnections {
    ...
    fn new(
        ...
        for n in 0..(send_data_migration.connections.get() - 1) {
...
fn send_migration_socket(
    ...
    if let Some(address) = send_data_migration.destination_url.strip_prefix("tcp:") {
        ...
        if send_data_migration.cert_pem.is_some() {
            Ok(SocketStream::Tcp(socket))
        } else {
            info!("Live Migration will be encrypted using TLS.");
            Ok(SocketStream::Tls(TlsStream::Client(
                tls::client_stream(
                    socket,
                    &send_data_migration.cert_pem.as_ref().unwrap(),
                    &address,
                )
                .unwrap(),
            )))
...
fn receive_migration_listener(
    ...
    if let Some(address) = receiver_data_migration.receiver_url.strip_prefix("tcp:") {
        ...
        if receiver_data_migration.cert_pem.is_some() {
            Ok(ReceiveListener::Tcp(listener))
        } else {
            Ok(ReceiveListener::Tls(
                listener,
                TlsConnectionWrapper::new(
                    &receiver_data_migration.cert_pem.as_ref().unwrap(),
                    &receiver_data_migration.key_pem.as_ref().unwrap(),
                ),
            ))
        }
