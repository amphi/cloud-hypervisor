...
/// Early VM state used during incoming migration restore.
///
/// This intentionally leaves out the device manager so the restore path can
/// prefault guest memory before the downtime-sensitive final state step.
pub(crate) struct RestoreVmShell {
    config: Arc<Mutex<VmConfig>>,
    vm: Arc<dyn hypervisor::Vm>,
    memory_manager: Arc<Mutex<MemoryManager>>,
    cpu_manager: Arc<Mutex<cpu::CpuManager>>,
...
impl Vm {
    ...
    ) -> Result<RestoreVmShell> {
        trace_scoped!("Vm::new_restore_shell");
